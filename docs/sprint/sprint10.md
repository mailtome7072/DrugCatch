...
### 잘 된 점 (Keep)
- Sprint 8에서 구축된 테스트 인프라(Playwright E2E 7개)를 새 스펙 파일 없이 `projects` 배열 추가만으로 모바일 viewport에서 재실행하는 방식이 매우 효율적이었다. 기존 코드 변경 없이 모바일 검증을 추가했다.
- `devices['iPhone SE']` Playwright 내장 descriptor를 사용하여 375×667 viewport, 터치 이벤트, 모바일 user-agent가 모두 사전 구성되었다. 별도 설정 없이 실제 모바일 환경에 가까운 조건에서 테스트할 수 있었다.
- Sprint 8에서 수동 검증 항목으로 남아 있던 모바일 반응형 동작을 자동화 검증으로 격상시켜 "⬜ 수동 검증 필요" 상태를 해소한 것 자체가 이 스프린트의 핵심 성과였다.
- `outputDir` + `screenshot: 'on'` 설정을 모바일 프로젝트에만 적용하여 스크린샷 저장이 모바일 실행 시에만 동작하도록 세밀하게 제어했다.

### 문제점 (Problem)
- `lighthouserc.json`의 `collect.settings`를 배열로 지정하는 문법이 `@lhci/cli` 버전에 따라 지원되지 않을 수 있다는 리스크가 식별되었으나, 실제 동작 여부 검증이 완료 기준에서 ⬜로 남겨졌다.
- `docs/sprint/sprint10/` 스크린샷 저장 결과가 검증 결과 테이블에서 "(검증 후 기록)"으로 남아 있어, 실제 Playwright 실행이 완료된 증거가 기록에 부재했다.
- 새 E2E 시나리오 작성을 "제외" 항목으로 명시하여 모바일 전용 인터랙션(터치 스와이프, 핀치 줌 등)은 검증 범위에서 제외되었다.
- Lighthouse CI 모바일 측정에서 `cpuSlowdownMultiplier: 1`로 설정하여 실제 모바일 저성능 환경을 충분히 에뮬레이션하지 않았다.

### 개선 방향 (Try)
- Playwright 모바일 E2E 실행 후 생성된 스크린샷 파일 목록을 즉시 문서화한다. CI에서 실행 결과를 artifact로 업로드하고 링크를 스프린트 문서에 기록한다.
- `lighthouserc.json` settings 배열 문법 유효성을 `npx @lhci/cli@0.14.x autorun --config=lighthouserc.json` 명령으로 로컬에서 먼저 검증한 후 CI에 적용한다.
- 모바일 터치 전용 인터랙션(파일 선택 버튼 탭, 미리보기 스크롤 등)을 `mobile-chrome` 프로젝트 전용 시나리오로 추가한다.

### 핵심 학습 (Key Learnings)
- Playwright의 `projects` 기반 다중 환경 테스트 전략은 동일한 시나리오를 desktop/mobile/tablet 등 여러 환경에서 재실행할 수 있어 테스트 코드 중복 없이 커버리지를 확장하는 가장 효율적인 방법이다.
- "⬜ 수동 검증 필요"로 남겨둔 검증 항목은 기술 부채가 된다. Sprint 10처럼 별도 스프린트를 할애하여 수동 검증을 자동화로 대체하는 것이 장기적으로 더 비용 효율적이다. 처음부터 자동화 가능한 검증은 자동화로 계획하는 습관이 필요하다.
