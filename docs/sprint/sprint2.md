...
- 이미지 업로드 클라이언트 컴포넌트 (`frontend/components/UploadPage.tsx`)
  ...
  - 이미지 재선택/교체: "다시 선택" 버튼으로 미리보기 초기화 후 재입력
  - "이미지 분석 시작" 버튼: 이미지 선택 전 비활성화, 선택 후 활성화
  - 로딩 인디케이터: 분석 요청 중 버튼 내 스피너 + 텍스트 "분석 중..." 표시
  - 에러 처리: 지원하지 않는 파일 형식, 파일 크기 초과(10MB) 안내
...
| 리스크 | 가능성 | 대응 방안 |
|--------|--------|----------|
| `capture="environment"` iOS Safari 미지원 가능성 | 저 | 미지원 브라우저에서는 파일 선택으로 폴백 (동일 input 재사용) |
...
- ✅ "카메라 촬영" 탭 선택 후 "촬영하기" 버튼 클릭 시 카메라가 열린다 (모바일)
- ✅ "파일 선택" 탭 선택 후 "파일 선택" 버튼 클릭 시 파일 선택창이 열린다
